...
## Question: How is cut related to color? 2 categorical vars
```{python}
alt.Chart(diamonds_grouped).mark_circle().encode(  # dot sizes
    x = 'color:N',
    y = 'cut:N',
    size='N:Q')
```
...
## Question: How is cut related to color? 2 categorical vars
```{python}
alt.Chart(diamonds_grouped).mark_rect().encode(  # color
    x = 'color:N',
    y = 'cut:N',
    color='N:Q')
```
...
```{python}
xy_size = alt.Chart(movies_url).mark_circle().encode(  # scatter plot
    alt.X('Rotten_Tomatoes_Rating:Q', bin=alt.BinParams(maxbins=20)),
    alt.Y('IMDB_Rating:Q', bin=alt.BinParams(maxbins=20)),
    alt.Size('count()')  # add size
)
xy_size
```
...
```{python}
xy_color = alt.Chart(movies_url).mark_bar().encode(  # bar plot
    alt.X('Rotten_Tomatoes_Rating:Q', bin=alt.BinParams(maxbins=20)),
    alt.Y('IMDB_Rating:Q', bin=alt.BinParams(maxbins=20)),
    alt.Color('count()')  # add color
) 
xy_color
```
